...
TEST_CASE("NonCompressedSimple") {
    ...
    TestType p{1, "hello"};
    REQUIRE(p.get_first() == 1);
    REQUIRE(p.get_second() == "hello");
}
...
TEST_CASE("NonCompressedModificationWorks") {
    ...
    p.get_first() = 5;
    p.get_second()[3] = 'k';
    REQUIRE(p.get_first() == 5);
    REQUIRE(p.get_second() == "helko");
}

TEST_CASE("SimpleOperationsWithCompressedPair") {
    CompressedPair<std::string, std::allocator<std::string>> p = {"hello",
                                                                  std::allocator<std::string>{}};
    std::string* ptr = new (p.get_second().allocate(1)) std::string{p.get_first()};
    REQUIRE(*ptr == "hello");
    ptr->~basic_string<char>();
    p.get_second().deallocate(ptr, 1);
}
...
TEST_CASE("Constructors") {
    ...
    REQUIRE(p0.get_first() == 0);
    REQUIRE(p0.get_second().empty());
...
TEST_CASE("ConstMethods") {
    ...
    REQUIRE(p1.get_second() == "hello!");

    const CompressedPair<int, MyStatelessFunctor> p2{1, MyStatelessFunctor{}};
    REQUIRE(p2.get_first() == 1);
    REQUIRE(p2.get_second()() == "I am so tired.");
}
