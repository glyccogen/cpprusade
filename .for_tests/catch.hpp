...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                                      \
	...
	struct create<Final, NttpTemplateTypeList<Containers...>, TypeList<Types...>> {            \
		using type =                                                                       \
		        typename append<Final<>, typename rewrap<NttpTemplateTypeList<Containers>, \
		                                                 Types...>::type...>::type;        \
	};
...
extern std::string const unprintableString;

std::string rawMemoryToString(void const * object, std::size_t size);

template <typename T> std::string rawMemoryToString(T const & object) {
...
template <typename T> class IsStreamInsertable {
    ...
    public:
	static bool const value = decltype(test<std::ostream, T const &>(0))::value;
};
...
template <typename T> std::string stringify(T const & e) {
...
template <> struct StringMaker<std::wstring> {
	static std::string convert(std::wstring const & wstr);
};
...
template <typename T1, typename T2> struct StringMaker<std::pair<T1, T2>> {
	static std::string convert(std::pair<T1, T2> const & pair) {
		ReusableStringStream rss;
...
template <typename T> struct StringMaker<std::optional<T>> {
	static std::string convert(std::optional<T> const & optional) {
		ReusableStringStream rss;
...
struct TupleElementPrinter {
	static void print(Tuple const & tuple, std::ostream & os) {
...
template <typename Tuple, std::size_t N> struct TupleElementPrinter<Tuple, N, false> {
	static void print(Tuple const &, std::ostream &) {}
};

} // namespace Detail

template <typename... Types> struct StringMaker<std::tuple<Types...>> {
	static std::string convert(std::tuple<Types...> const & tuple) {
		ReusableStringStream rss;
		rss << '{';
...
template <> struct StringMaker<std::monostate> {
	static std::string convert(std::monostate const &) {
		return "{ }";
	}
};

template <typename... Elements> struct StringMaker<std::variant<Elements...>> {
	static std::string convert(std::variant<Elements...> const & variant) {
...
struct StringMaker<std::chrono::time_point<std::chrono::system_clock, Duration>> {
	...
	convert(std::chrono::time_point<std::chrono::system_clock, Duration> const & time_point) {
		...
		char const * const fmt = "%Y-%m-%dT%H:%M:%SZ";
...
#pragma warning(disable                                                                            \
                : 4312) // Converting int to T* using reinterpret_cast (issue on x64 platform)
...
struct IMutableRegistryHub {
	...
	virtual void registerTranslator(IExceptionTranslator const * translator) = 0;
...
namespace Detail {
std::string finalizeDescription(std::string const & desc);
}
...
class GeneratorException : public std::exception {
    ...
    public:
	GeneratorException(char const * msg) : m_msg(msg) {}
...
template <typename T> class Generators : public IGenerator<T> {
	...
	bool next() override {
		...
		bool const current_status = m_generators[m_current].next();
		if(!current_status) {
			++m_current;
		}
...
template <typename T> class TakeGenerator : public IGenerator<T> {
	...
	bool next() override {
		...
		auto const success = m_generator.next();
...
template <typename T> class RepeatGenerator : public IGenerator<T> {
	...
	bool next() override {
		...
		// In the second case, we need to move indices forward and check that we haven't run
		// up against the end
		++m_repeat_index;
...
class SimplePcg32 {
	...
	static std::uint64_t const s_inc = (0x13ed0cc53f939476ULL << 1ULL) | 1ULL;
};
		...
		Class cls = classes[c];
		{
			...
			for(u_int m = 0; m < count; m++) {
				...
				if(startsWith(methodName, "Catch_TestCase_")) {
					...
					std::string desc = Detail::getAnnotation(cls, "Description",
					                                         testCaseName);
...
class TestSpec {
	...
	vectorStrings const & getInvalidArgs() const;
...
std::string const benchmarkErrorMsg = "a benchmark failed to run successfully";
} // namespace Detail
...
struct BenchmarkFunction {
	...
	template <typename Fun, typename std::enable_if<!is_related<Fun, BenchmarkFunction>::value,
	                                                int>::type = 0>
...
template <typename Duration> struct ExecutionPlan {
	...
	template <typename Clock>
	std::vector<FloatDuration<Clock>> run(IConfig const & cfg,
...
auto const warmup_iterations = 10000;
auto const warmup_time = std::chrono::milliseconds(100);
auto const minimum_ticks = 1000;
auto const warmup_seed = 10000;
auto const clock_resolution_estimation_time = std::chrono::milliseconds(500);
auto const clock_cost_estimation_time_limit = std::chrono::seconds(1);
auto const clock_cost_estimation_tick_limit = 100000;
auto const clock_cost_estimation_time = std::chrono::milliseconds(10);
auto const clock_cost_estimation_iterations = 10000;
...
EnvironmentEstimate<FloatDuration<Clock>> estimate_clock_cost(FloatDuration<Clock> resolution) {
	...
	auto time_clock = [](int k) {
		return Detail::measure<Clock>([k] {
			       for(int i = 0; i < k; ++i) {
				       auto volatile ignored = Clock::now();
				       (void)ignored;
			       }
		       })
		        .elapsed;
...
SampleAnalysis<Duration> analyse(IConfig const & cfg, Environment<Duration>, Iterator first,
                                 Iterator last) {
...
struct Benchmark {
	...
	template <typename Clock>
	ExecutionPlan<FloatDuration<Clock>> prepare(IConfig const & cfg,
	                                            Environment<FloatDuration<Clock>> env) const {
		auto min_time = env.clock_resolution.mean * Detail::minimum_ticks;
		auto run_time = std::max(min_time, std::chrono::duration_cast<decltype(min_time)>(
		...
		return {new_iters,
		        test.elapsed / test.iterations * new_iters * cfg.benchmarkSamples(), fun,
	...
	template <typename Fun, typename std::enable_if<!Detail::is_related<Fun, Benchmark>::value,
	                                                int>::type = 0>
...
template <typename T, bool Destruct> struct ObjectStorage {
	...
	ObjectStorage(ObjectStorage const & other) {
...
double normal_quantile(double p) {
	static double const ROOT_TWO = std::sqrt(2.0);
...
bool Approx::equalityComparisonImpl(double const other) const {
...
class RunContext : public IResultCapture, public IRunner {
	...
	AssertionResult const * getLastResult() const override;
...
struct UnaryLambdaTraits<ReturnT (ClassT::*)(ArgT) const> {
	static bool const isValid = true;
	using ArgType =
...
inline auto invokeLambda(L const & lambda, std::string const & arg) -> ParserResult {
	...
	return !result ? result
	               : LambdaInvoker<typename UnaryLambdaTraits<L>::ReturnType>::invoke(lambda,
	                                                                                  temp);
}
...
struct Parser : ParserBase {
	...
	auto parse(std::string const & exeName, TokenStream const & tokens) const
		...
		size_t const totalParsers = m_options.size() + m_args.size();
...
clara::Parser makeCommandLineParser(ConfigData & config) {
	...
	auto cli =
	        ...
	        Opt(config.minDuration,
	            "seconds")["-D"]["--min-duration"]("show test durations for tests taking at "
	                                               "least the given number of seconds") |
...
class PosixColourImpl : public IColourImpl {
    ...
    private:
	void setColour(char const * _escapeCode) {
...
std::unique_ptr<EnumInfo> makeEnumInfo(StringRef enumName, StringRef allValueNames,
	...
	auto const valueNames = Catch::Detail::parseEnums(allValueNames);
...
class ExceptionTranslatorRegistry : public IExceptionTranslatorRegistry {
	...
	virtual void registerTranslator(IExceptionTranslator const * translator);
...
void ExceptionTranslatorRegistry::registerTranslator(IExceptionTranslator const * translator) {
	m_translators.push_back(std::unique_ptr<IExceptionTranslator const>(translator));
}
...
char const * GeneratorException::what() const noexcept {
	return m_msg;
}
...
bool WithinRelMatcher::match(double const & matchee) const {
	auto const relMargin = m_epsilon * (std::max)(std::fabs(matchee), std::fabs(m_target));
...
std::string Catch::Matchers::Generic::Detail::finalizeDescription(std::string const & desc) {
	if(desc.empty()) {
...
SimplePcg32::result_type SimplePcg32::operator()() {
	...
	uint32_t const xorshifted = static_cast<uint32_t>(((m_state >> 18u) ^ m_state) >> 27u);
	auto const output = rotate_right(xorshifted, m_state >> 59u);
...
class RegistryHub : public IRegistryHub, public IMutableRegistryHub, private NonCopyable {
	...
	void registerTranslator(IExceptionTranslator const * translator) override {
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
	...
	void close() override {
		...
		bool const should_wait_for_child = [&]() {
...
AssertionResult const * RunContext::getLastResult() const {
	return &(*m_lastResult);
}
...
Session::Session() {
	...
	if(!exceptions.empty()) {
		...
		for(auto const & ex_ptr : exceptions) {
			try {
...
StringRef trim(StringRef ref) {
	auto const is_ws = [](char c) { return c == ' ' || c == '\t' || c == '\n' || c == '\r'; };
	size_t real_begin = 0;
...
struct TestHasher {
	...
	uint32_t operator()(TestCase const & t) const {
		...
		hash_t const prime = 1099511628211u;
		hash_t hash = 14695981039346656037u;
		for(char const c : t.name) {
			hash ^= c;
			hash *= prime;
		}
		hash ^= m_hashSuffix;
		hash *= prime;
		uint32_t const low{static_cast<uint32_t>(hash)};
		uint32_t const high{static_cast<uint32_t>(hash >> 32)};
		return low * high;
	}
...
TestSpec::vectorStrings const & TestSpec::getInvalidArgs() const {
...
auto estimateClockResolution() -> uint64_t {
	uint64_t sum = 0;
	static uint64_t const iterations = 1000000;
...
std::string const unprintableString = "{?}";

namespace {
int const hexThreshold = 255;
...
std::string StringMaker<std::string>::convert(std::string const & str) {
...
std::string getFormattedDuration(double duration) {
	...
	std::size_t const maxDoubleSize = DBL_MAX_10_EXP + 1 + 1 + 3 + 1;
...
bool shouldShowDuration(IConfig const & config, double duration) {
	...
	double const min = config.minDuration();
...
void printTotals(std::ostream & out, Totals const & totals) {
	if(totals.testCases.total() == 0) {
		out << "No tests ran.";
	} else if(totals.testCases.failed == totals.testCases.total()) {
		Colour colour(Colour::ResultError);
		std::string const qualify_assertions_failed =
...
class AssertionPrinter {
	...
	void printRemainingMessages(Colour::Code colour = dimColour()) {
		...
		auto const itEnd = messages.cend();
		auto const N = static_cast<std::size_t>(std::distance(itMessage, itEnd));
...
class ConsoleAssertionPrinter {
	...
	ConsoleAssertionPrinter(std::ostream & _stream, AssertionStats const & _stats,
	        ...
	        : stream(_stream), stats(_stats), result(_stats.assertionResult),
	          colour(Colour::None), message(result.getMessage()), messages(_stats.infoMessages),
...
class Duration {
	...
	static uint64_t const s_nanosecondsInAMicrosecond = 1000;
	static uint64_t const s_nanosecondsInAMillisecond = 1000 * s_nanosecondsInAMicrosecond;
	static uint64_t const s_nanosecondsInASecond = 1000 * s_nanosecondsInAMillisecond;
	static uint64_t const s_nanosecondsInAMinute = 60 * s_nanosecondsInASecond;
...
class TablePrinter {
	...
	friend TablePrinter & operator<<(TablePrinter & tp, ColumnBreak) {
		...
		auto const strSize = colStr.size();
		tp.m_oss.str("");
		tp.open();
...
std::string fileNameTag(std::vector<std::string> const & tags) {
...
#define CATCH_TEMPLATE_TEST_CASE_METHOD(className, ...)                                            \
	INTERNAL_CATCH_EXPAND_VARGS(                                                               \
	        INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION(className, __VA_ARGS__))
#define CATCH_TEMPLATE_TEST_CASE_METHOD_SIG(className, ...)                                        \
	INTERNAL_CATCH_EXPAND_VARGS(INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_SIG_NO_REGISTRATION(  \
	        className, __VA_ARGS__))
#define CATCH_TEMPLATE_PRODUCT_TEST_CASE(...) CATCH_TEMPLATE_TEST_CASE(__VA_ARGS__)
#define CATCH_TEMPLATE_PRODUCT_TEST_CASE_SIG(...) CATCH_TEMPLATE_TEST_CASE(__VA_ARGS__)
#define CATCH_TEMPLATE_PRODUCT_TEST_CASE_METHOD(className, ...)                                    \
